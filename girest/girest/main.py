...
class GIRest:
    pointer_schema = {"type": "string", "pattern": "^0x[0-9a-fA-F]+$|^[0-9]+$"}
    event_schema = {"type": "object", "required": ["data"], "properties": {"data": {"type": "string"}}}
    ...
    def _add_callback_parameters(self, operation):
    ...
    def _generate_function(self, bim, bi=None, is_constructor=False, is_destructor=False, is_copy=False):
        ...
        for i in range(n_args):
            ...
            if tag == "interface":
                ...
                if interface and interface.get_type() == GIRepository.InfoType.CALLBACK:
                    ...
                    # Generate full callback specification
            ...
            if arg_direction == GIRepository.Direction.OUT:
                ...
                if tag != "interface" or (
                    interface.get_type() in [GIRepository.InfoType.STRUCT, GIRepository.InfoType.OBJECT]
                    and not GIRepository.arg_info_is_caller_allocates
                ):
        ...
        if response_props:
            responses["200"] = {
                "description": "Success",
                "content": {"application/json": {"schema": {"type": "object", "properties": response_props}}},
            }
        else:
        ...
        if method_callbacks:
            ...
            self._add_callback_parameters(operation)
    ...
    def _generate_object(self, bi):
        ...
        for i in range(0, GIRepository.object_info_get_n_methods(bi)):
            ...
            if ref_func_name and method_name == ref_func_name:
                ...
                logger.warning(
                    f"Using fallback detection for ref function '{method_name}' in {bi.get_namespace()}.{bi.get_name()}"
                )
                is_copy = True
                found_ref_func = True
            ...
            if unref_func_name and method_name == unref_func_name:
                ...
                logger.warning(
                    f"Using fallback detection for unref function '{method_name}' in {bi.get_namespace()}.{bi.get_name()}"
                )
                is_destructor = True
                found_unref_func = True
        ...
        if not found_ref_func and ref_func_name:
            logger.warning(
                f"Ref function '{ref_func_name}' not found in methods of {bi.get_namespace()}.{bi.get_name()}"
            )
        if not found_unref_func and unref_func_name:
            logger.warning(
                f"Unref function '{unref_func_name}' not found in methods of {bi.get_namespace()}.{bi.get_name()}"
            )
        # The type function
    ...
    def _generate_struct(self, bi, generate_class=False, class_of=None):
        ...
        if bi.get_name() in ["ObjectClass"] and bi.get_namespace() == "GObject":
            ...
            if n_fields > 0:
                ...
                if tag == "interface":
                    ...
                    if (
                        interface.get_type() == GIRepository.InfoType.STRUCT
        ...
        schema = {"x-gi-type": "struct", "x-gi-namespace": f"{bi.get_namespace()}", "x-gi-name": f"{bi.get_name()}"}
        ...
        self.spec.components.schema(full_name, schema)
        ...
        for i in range(0, n_methods):
            ...
            if copy_func_name and method_name == copy_func_name:
                ...
                logger.warning(
                    f"Using fallback detection for copy function '{method_name}' in {bi.get_namespace()}.{bi.get_name()}"
                )
                is_copy = True
            ...
            if free_func_name and method_name == free_func_name:
                ...
                logger.warning(
                    f"Using fallback detection for free function '{method_name}' in {bi.get_namespace()}.{bi.get_name()}"
                )
                is_destructor = True

            self._generate_function(
                bim, bi, is_constructor=is_constructor, is_copy=is_copy, is_destructor=is_destructor
            )
    ...
    def _generate_callback(self, bi, schema_name=None, emitter_info=None):
        ...
        if emitter_info is not None:
            ...
            request_properties["self"] = {"$ref": f"#/components/schemas/{emitter_full_name}", "x-gi-transfer": "none"}
        ...
        for i in range(n_args):
            ...
            param_with_transfer = {**param_schema, "x-gi-transfer": transfer_str}
        ...
        # Create a separate return value schema for HTTP responses
        # This includes the return value AND any output parameters
        if return_schema or response_properties:
            ...
            return_wrapper_schema = {"type": "object", "properties": response_properties}
            self.spec.components.schema(return_schema_name, return_wrapper_schema)

        # Add HTTP callback invocation properties
        # These are used when the callback is invoked via HTTP POST (request body)
        request_properties["sessionId"] = {"type": "string", "description": "Session identifier for routing"}
        request_properties["callbackName"] = {
            "type": "string",
            "description": "Name of the callback being invoked",
        }
        request_properties["args"] = {"type": "array", "description": "Callback arguments in order", "items": {}}
        request_properties["invocationNumber"] = {"type": "integer", "description": "Sequential invocation counter"}
    ...
    def _generate_callback_argument(self, callback_info, arg_info, callback_arg_name):
        ...
        callback_param = {
            ...
            "in": "query",
            "required": True,
            "schema": {"type": "string", "format": "uri", "description": f"URL to invoke for {callback_name} callback"},
        ...
        if is_sync:
            ...
            if return_schema:
                ...
                responses = {
                    "200": {
                        ...
                        "content": {"application/json": {"schema": {"$ref": return_schema_ref}}},
                    },
                    "400": {"description": "Invalid callback request"},
                    "401": {"description": "Invalid signature or authentication failed"},
                    "500": {"description": "Callback processing error"},
                }
            else:
                # No return value: respond with 204 No Content
                responses = {
                    "204": {"description": "Callback processed successfully (no content)"},
                    "400": {"description": "Invalid callback request"},
                    "401": {"description": "Invalid signature or authentication failed"},
                    "500": {"description": "Callback processing error"},
                }
            ...
            responses = {
                "204": {"description": "Callback received (no content)"},
                "400": {"description": "Invalid callback request"},
                "401": {"description": "Invalid signature or authentication failed"},
                "500": {"description": "Callback processing error"},
            }
            ...
            callback_name: {
                "{$request.query." + callback_param["name"] + "}": {
                    "post": {
                        ...
                        "requestBody": {
                            "required": True,
                            "content": {"application/json": {"schema": {"$ref": callback_schema_ref}}},
                        },
                        "responses": responses,
                ...
                "200": {
                    ...
                    "content": {
                        "application/json": {
                            "schema": {
                                "type": "object",
                                "properties": {"return": {"$ref": f"#/components/schemas/{namespace}{name}"}},
                            }
                        }
                    },
                }
        ...
        operation = {
            ...
            "responses": {
                "200": {
                    "description": "Success",
                    "content": {
                        "application/json": {"schema": {"type": "object", "properties": {"return": {"type": "string"}}}}
                    },
                }
            },
        }
                ...
                "parameters": [
                    {
                        ...
                        "in": "path",
                        "required": True,
                        "schema": {"$ref": f"#/components/schemas/{full_name}"},
                    ...
                    "200": {
                        ...
                        "content": {
                            "application/json": {
                                "schema": {
                                    "type": "object",
                                    "properties": {"return": {"type": "number", "description": "Signal handler ID"}},
                                }
                            }
                        },
                    }
                    ...
                    "{$request.body#/handler}": {
                        "post": {
                            ...
                            "requestBody": {
                                ...
                                "content": {
                                    "application/json": {
                                        "schema": {"$ref": f"#/components/schemas/{callback_schema_name}"}
                                    }
                                },
                            },
                            "responses": {
    ...
    def _generate_missing(self, ns):
        ...
        if ns == "GObject":
            post_api = "/GObject/signal_connect_data"
            post_operation = {
                "summary": "Connects a callback function to a signal for a particular object",
                "description": "",
                "operationId": "GObject--signal_connect_data",
                "tags": ["GObject"],
                "parameters": [],  # Empty list that will be populated by _add_callback_parameters if needed
                "requestBody": {
                    "required": True,
                    "content": {
                        "application/json": {
                    ...
                    "200": {
                        ...
                        "content": {
                            "application/json": {
                                "schema": {
                                    "type": "object",
                                    "properties": {"return": {"type": "number", "description": "Handler ID"}},
                                }
                            }
                        },
                    }
            ...
            self._add_callback_parameters(post_operation)
                    ...
                    "{$request.body#/c_handler}": {
                        "post": {
                            ...
                            "requestBody": {
                                "required": True,
                                "content": {
                                    "application/json": {"schema": {"$ref": "#/components/schemas/GObjectCallback"}}
                                },
                            },
                            "responses": {
                                "204": {"description": "Callback received (no content)"},
