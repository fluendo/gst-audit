...
class CallbackSecurity:
    ...
    def verify_signature(self, payload: Dict[str, Any], timestamp: str, provided_signature: str) -> bool:
        """
...
class CallbackHandler:
    ...
    def __init__(self, callback_url: Optional[str], session_id: str, secret: str, timeout: int = 10):
        """
        ...
        # Statistics
        self.stats = {"total_invocations": 0, "successful_invocations": 0, "failed_invocations": 0, "total_time_ms": 0}
    ...
    def _post_callback(self, payload: Dict[str, Any], expect_response: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Callback posted successfully ({elapsed_ms:.2f}ms): " f"{payload.get('callbackName', 'unknown')}"
            )
            ...
            logger.error(f"Callback timeout after {self.timeout}s: " f"{payload.get('callbackName', 'unknown')}")
            self.stats["failed_invocations"] += 1
            return None

        except requests.exceptions.RequestException as e:
            logger.error(f"Callback request failed: {e} " f"({payload.get('callbackName', 'unknown')})")
            self.stats["failed_invocations"] += 1
            return None

        except Exception as e:
            logger.error(f"Unexpected error in callback: {e} " f"({payload.get('callbackName', 'unknown')})")
            self.stats["failed_invocations"] += 1
            return None
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_invocations"] > 0:
            ...
            stats["avg_time_ms"] = stats["total_time_ms"] / stats["total_invocations"]
        else:
