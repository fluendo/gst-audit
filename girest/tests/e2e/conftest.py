...
def assert_api_success(response, msg="API call failed"):
    ...
    assert 200 <= response.status_code < 300, \
        f"{msg}: {response.status_code}, response: {response.text}"
...
def gst_pipeline():
    ...
    try:
        ...
        print(f"⚠ Pipeline didn't terminate gracefully, killing it")
        process.kill()
        process.wait()
...
def _start_girest_server(gst_pipeline, sse_only=False, port=9000):
    ...
    cmd = ["python3", "-u", girest_path, "Gst", "1.0", 
           "--pid", str(gst_pipeline), "--port", str(port)]
    if sse_only:
        cmd.append("--sse-only")
    ...
    while time.time() - start_time < timeout:
        ...
        if line:
            ...
            if "Uvicorn running on" in line:
                ready = True
                print(f"✓ GIRest server ready (detected startup message)")
                break
    ...
    async def create_bin(name=None):
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            response = await client.get(
                f"{girest_server}/Gst/ElementFactory/make",
                params=params
            )
