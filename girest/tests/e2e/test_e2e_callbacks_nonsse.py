    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        response = await client.get(
            ...
            headers={
                ...
                "callback-secret": "test-secret-456"
            }
        )
        ...
        assert len(received_pads) == 2, \
            f"Expected 2 pad callbacks (continued iteration), got {len(received_pads)}"
        
        # Verify both pads are different
        assert received_pads[0] != received_pads[1], \
            f"Expected different pads, got same: {received_pads[0]}"
        
        # Verify all callbacks are stored
        all_callbacks = callback_server.get_callbacks("foreach_pad_test")
        assert len(all_callbacks) == 2, \
            f"Expected 2 callbacks stored, got {len(all_callbacks)}"
...
async def test_call_scope_stops_on_false(girest_server, callback_server, gst_identity_factory):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        assert len(received_pads) == 1, \
            f"Expected 1 pad callback (stopped iteration), got {len(received_pads)}"
        
        # Verify only one callback is stored
        all_callbacks = callback_server.get_callbacks("foreach_pad_test")
        assert len(all_callbacks) == 1, \
            f"Expected 1 callback stored, got {len(all_callbacks)}"
...
async def test_call_scope_reentrancy(girest_server, callback_server, gst_identity_factory):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        def reentrant_callback_handler(callback_data):
            ...
            try:
                ref_response = httpx.get(
                    f"{girest_server}/Gst/Object/ptr,{pad_ptr}/ref",
                    timeout=10.0
                )
                ...
                unref_response = httpx.get(
                    f"{girest_server}/Gst/Object/ptr,{pad_ptr}/unref",
                    timeout=10.0
                )
        ...
        assert len(received_pads) == 2, \
            f"Expected 2 pad callbacks, got {len(received_pads)}"
        ...
        assert len(reentrant_calls_succeeded) == 2, \
            f"Expected 2 reentrant call attempts, got {len(reentrant_calls_succeeded)}"
        assert all(reentrant_calls_succeeded), \
            "Some reentrant API calls failed"
...
async def test_async_scope(girest_server, callback_server, gst_bin_factory, gst_identity_factory):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        def signal_handler(callback_data):
            ...
            assert args["self"]["ptr"] == bin_ptr, \
                f"Signal 'self' parameter should be bin {bin_ptr}, got {args['self']['ptr']}"
            
            signal_triggered.append({
                "self": args["self"]["ptr"],
                "element": args["element"]["ptr"]
            })
        ...
        response = await client.post(
            ...
            headers={
                "session-id": "test-session-signal",
                "callback-secret": "test-secret-signal"
            }
        )
        ...
        assert len(signal_triggered) == 1, \
            f"Expected 1 signal trigger, got {len(signal_triggered)}"
        assert signal_triggered[0]["element"] == identity1_ptr, \
            "Signal element pointer doesn't match identity1"
        
        # Step 6: Remove the signal handler
        response = await client.get(
            f"{girest_server}/GObject/signal_handler_disconnect",
            params={
                "instance": f"ptr,{bin_ptr}",
                "handler_id": signal_id
            }
        )
