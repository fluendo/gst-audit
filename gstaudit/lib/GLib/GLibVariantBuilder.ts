...
export class GLibVariantBuilder {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibVariantBuilder> {
    const instance = new GLibVariantBuilder(ptr, transferType);
    if (transferType === 'full')
      glibvariantbuilderRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibVariantBuilder.create(data.return.ptr, 'full');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async add_value(value_: GLibVariant, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
...
return Promise.reject("Call failed");
  ...
  async open(type_: GLibVariantType, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
