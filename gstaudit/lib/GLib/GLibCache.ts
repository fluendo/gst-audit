...
export class GLibCache {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibCache> {
    const instance = new GLibCache(ptr, transferType);
    if (transferType === 'full')
      glibcacheRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibCache.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async key_foreach(session_id: string, callback_secret: string, func: GLibHFunc, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async remove(value_?: Pointer, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async value_foreach(session_id: string, callback_secret: string, func: GLibHFunc, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
