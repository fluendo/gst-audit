...
export class GLibRand {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibRand> {
    const instance = new GLibRand(ptr, transferType);
    if (transferType === 'full')
      glibrandRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibRand.create(data.return.ptr, 'full');
  return instance;
}
...
return Promise.reject("Call failed");
  ...
  async set_seed(seed: number, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async set_seed_array(seed: number, seed_length: number, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
