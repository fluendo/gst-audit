...
import type { GLibIOError } from './GLibIOError';
import type { GLibIOFlags } from './GLibIOFlags';
import { GLibIOFuncs } from './GLibIOFuncs';
import type { GLibIOStatus } from './GLibIOStatus';
...
export class GLibIOChannel {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibIOChannel> {
    const instance = new GLibIOChannel(ptr, transferType);
    if (transferType === 'full')
      glibiochannelRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibIOChannel.create(data.return.ptr, 'full');
  return instance;
}
...
return Promise.reject("Call failed");
  ...
  async close(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async read_chars(): Promise<{buf: Array<number>, bytes_read: number, return: GLibIOStatus}> {
    // Increment ref for parameters with full transfer ownership
    ...
    try {
      ...
      result.buf = await (async () => {
        if (data.buf && Array.isArray(data.buf)) {
  ...
  async read_line(): Promise<{str_return: string, length: number, terminator_pos: number, return: GLibIOStatus}> {
    ...
    try {
      ...
      result.terminator_pos = await (async () => {
  ...
  async read_to_end(): Promise<{str_return: Array<number>, return: GLibIOStatus}> {
    // Increment ref for parameters with full transfer ownership
    ...
    try {
      ...
      // Handle return parameter: str_return
      result.str_return = await (async () => {
        if (data.str_return && Array.isArray(data.str_return)) {
          // Array of basic types - return as-is
          return data.str_return;
        }
        return Promise.reject("Call failed");
...
return Promise.reject("Call failed");
  ...
  async set_buffer_size(size: number, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async set_buffered(buffered: boolean, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async set_close_on_unref(do_close: boolean, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async set_line_term(length: number, line_term?: string, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async write_chars(buf: Array<number>, count: number): Promise<{bytes_written: number, return: GLibIOStatus}> {
    // Increment ref for parameters with full transfer ownership
...
return Promise.reject("Call failed");
  ...
  async get_partial_write_buf(): Promise<Array<number>> {
    // Increment ref for parameters with full transfer ownership
