...
export class GLibRWLock {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibRWLock> {
    const instance = new GLibRWLock(ptr, transferType);
    if (transferType === 'full')
      glibrwlockRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibRWLock.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async reader_lock(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async reader_unlock(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async writer_lock(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async writer_unlock(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
