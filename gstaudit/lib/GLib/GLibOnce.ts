...
import type { GLibOnceStatus } from './GLibOnceStatus';
...
export class GLibOnce {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibOnce> {
    const instance = new GLibOnce(ptr, transferType);
    if (transferType === 'full')
      glibonceRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibOnce.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  static async init_leave_pointer(location: Pointer, result_?: Pointer, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
