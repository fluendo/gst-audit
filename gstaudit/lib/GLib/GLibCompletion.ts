...
import type { GLibCompletionFunc } from './GLibCompletionFunc';
import { convertGLibCompletionFuncArgs } from './GLibCompletionFunc';
import type { GLibCompletionStrncmpFunc } from './GLibCompletionStrncmpFunc';
import { convertGLibCompletionStrncmpFuncArgs } from './GLibCompletionStrncmpFunc';
...
export class GLibCompletion {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibCompletion> {
    const instance = new GLibCompletion(ptr, transferType);
    if (transferType === 'full')
      glibcompletionRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibCompletion.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async clear_items(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
...
if (data.return && typeof data.return === 'object' && 'ptr' in data.return) {
  const instance = await GLibList.create(data.return.ptr, 'none', string, undefined);
  return instance;
}
return Promise.reject("Call failed");
