...
export class GLibTestLogBuffer {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibTestLogBuffer> {
    const instance = new GLibTestLogBuffer(ptr, transferType);
    if (transferType === 'full')
      glibtestlogbufferRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibTestLogBuffer.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async push(n_bytes: number, bytes: number, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
...
const instance = await GLibString.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
