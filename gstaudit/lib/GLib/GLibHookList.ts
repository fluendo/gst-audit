...
import type { GLibHookCheckMarshaller } from './GLibHookCheckMarshaller';
import { convertGLibHookCheckMarshallerArgs } from './GLibHookCheckMarshaller';
import type { GLibHookFinalizeFunc } from './GLibHookFinalizeFunc';
import { convertGLibHookFinalizeFuncArgs } from './GLibHookFinalizeFunc';
import type { GLibHookMarshaller } from './GLibHookMarshaller';
import { convertGLibHookMarshallerArgs } from './GLibHookMarshaller';
...
export class GLibHookList {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibHookList> {
    const instance = new GLibHookList(ptr, transferType);
    if (transferType === 'full')
      glibhooklistRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibHookList.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async init(hook_size: number, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async invoke(may_recurse: boolean, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async invoke_check(may_recurse: boolean, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async marshal(may_recurse: boolean, session_id: string, callback_secret: string, marshaller: GLibHookMarshaller, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
    ...
    let marshaller_callbackInfo: { callbackUrl: string; callbackId: string } | undefined;
    if (typeof marshaller !== 'undefined') {
      ...
      marshaller_callbackInfo = callbackHandler.registerCallback(
        marshaller, 
        convertGLibHookMarshallerArgs,
        {
          methodName: 'marshal',
          paramName: 'marshaller'
        }
      );
      
      // Pass the callback URL to the server
      url.searchParams.append('marshaller', marshaller_callbackInfo.callbackUrl);
    }
    try {
      // Add headers for callback authentication
      const headers: Record<string, string> = {};
  ...
  async marshal_check(may_recurse: boolean, session_id: string, callback_secret: string, marshaller: GLibHookCheckMarshaller, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
    ...
    if (typeof marshaller !== 'undefined') {
      ...
      marshaller_callbackInfo = callbackHandler.registerCallback(
        marshaller, 
        convertGLibHookCheckMarshallerArgs,
        {
          methodName: 'marshal_check',
          paramName: 'marshaller'
        }
      );
