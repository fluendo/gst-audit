...
export class GLibHashTableIter {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibHashTableIter> {
    const instance = new GLibHashTableIter(ptr, transferType);
    if (transferType === 'full')
      glibhashtableiterRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibHashTableIter.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async init(hash_table: Pointer, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async remove(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async steal(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
