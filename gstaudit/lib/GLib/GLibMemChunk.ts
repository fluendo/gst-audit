...
export class GLibMemChunk {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibMemChunk> {
    const instance = new GLibMemChunk(ptr, transferType);
    if (transferType === 'full')
      glibmemchunkRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibMemChunk.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async free(mem?: Pointer, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async clean(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  static async info(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
