...
export class GLibTimer {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibTimer> {
    const instance = new GLibTimer(ptr, transferType);
    if (transferType === 'full')
      glibtimerRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibTimer.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async continue_(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
