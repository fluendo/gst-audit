...
export class GLibRelation {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibRelation> {
    const instance = new GLibRelation(ptr, transferType);
    if (transferType === 'full')
      glibrelationRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibRelation.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async print(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
