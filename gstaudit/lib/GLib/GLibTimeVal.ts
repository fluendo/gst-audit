...
export class GLibTimeVal {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibTimeVal> {
    const instance = new GLibTimeVal(ptr, transferType);
    if (transferType === 'full')
      glibtimevalRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibTimeVal.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async add(microseconds: number, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
