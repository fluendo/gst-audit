...
export class GLibVariantType {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibVariantType> {
    const instance = new GLibVariantType(ptr, transferType);
    if (transferType === 'full')
      glibvarianttypeRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibVariantType.create(data.return.ptr, 'full');
  return instance;
}
...
return Promise.reject("Call failed");
  ...
  static async new_tuple(items: Array<GLibVariantType>): Promise<GLibVariantType> {
    // Increment ref for parameters with full transfer ownership
