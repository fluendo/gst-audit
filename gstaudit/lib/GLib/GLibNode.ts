...
import type { GLibNodeForeachFunc } from './GLibNodeForeachFunc';
import { convertGLibNodeForeachFuncArgs } from './GLibNodeForeachFunc';
import type { GLibNodeTraverseFunc } from './GLibNodeTraverseFunc';
import { convertGLibNodeTraverseFuncArgs } from './GLibNodeTraverseFunc';
import type { GLibTraverseFlags } from './GLibTraverseFlags';
...
export class GLibNode {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GLibNode> {
    const instance = new GLibNode(ptr, transferType);
    if (transferType === 'full')
      glibnodeRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GLibNode.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async children_foreach(flags: GLibTraverseFlags, session_id: string, callback_secret: string, func: GLibNodeForeachFunc, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
    ...
    if (typeof func !== 'undefined') {
      ...
      func_callbackInfo = callbackHandler.registerCallback(
        func, 
        convertGLibNodeForeachFuncArgs,
        {
          methodName: 'children_foreach',
          paramName: 'func'
        }
      );
  ...
  async reverse_children(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async traverse(order: GLibTraverseType, flags: GLibTraverseFlags, max_depth: number, session_id: string, callback_secret: string, func: GLibNodeTraverseFunc, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
    ...
    if (typeof func !== 'undefined') {
      ...
      func_callbackInfo = callbackHandler.registerCallback(
        func, 
        convertGLibNodeTraverseFuncArgs,
        {
          methodName: 'traverse',
          paramName: 'func'
        }
  ...
  async unlink(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
