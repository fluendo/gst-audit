...
import type { GstBufferListFunc } from './GstBufferListFunc';
import { convertGstBufferListFuncArgs } from './GstBufferListFunc';
...
export class GstBufferList {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstBufferList> {
    const instance = new GstBufferList(ptr, transferType);
    if (transferType === 'full')
      gstbufferlistRegistry.register(instance, ptr);
    return instance;
  }
...
return Promise.reject("Call failed");
  ...
  async foreach(session_id: string, callback_secret: string, func: GstBufferListFunc): Promise<boolean> {
    // Increment ref for parameters with full transfer ownership
...
return Promise.reject("Call failed");
  ...
  async insert(idx: number, buffer: GstBuffer, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async remove(idx: number, length: number, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
