...
export class GstUri {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstUri> {
    const instance = new GstUri(ptr, transferType);
    if (transferType === 'full')
      gsturiRegistry.register(instance, ptr);
    return instance;
  }
...
if (data.return && typeof data.return === 'object' && 'ptr' in data.return) {
  const instance = await GLibList.create(data.return.ptr, 'full', string, undefined);
  return instance;
}
return Promise.reject("Call failed");
  ...
  async get_query_keys(): Promise<GLibList> {
    ...
    // Return value is a List, instantiate it from the ptr
if (data.return && typeof data.return === 'object' && 'ptr' in data.return) {
  const instance = await GLibList.create(data.return.ptr, 'container', string, undefined);
  return instance;
}
return Promise.reject("Call failed");
