...
export class GstElementFactory extends GstPluginFeature {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstElementFactory> {
    const instance = new GstElementFactory(ptr, transferType);
...
const instance = await GstElementFactory.create(data.return.ptr, 'full');
        return instance;
      }
...
if (data.return && typeof data.return === 'object' && 'ptr' in data.return) {
  const instance = await GLibList.create(data.return.ptr, 'full', GstElementFactory, undefined);
  return instance;
}
return Promise.reject("Call failed");
...
const instance = await GstElement.create(data.return.ptr, 'none');
  ...
  static async make_with_properties(factoryname: string, names?: Array<string>, values?: Array<GObjectValue>): Promise<GstElement | null> {
    // Increment ref for parameters with full transfer ownership
...
const instance = await GstElement.create(data.return.ptr, 'none');
  ...
  async create_with_properties(names?: Array<string>, values?: Array<GObjectValue>): Promise<GstElement | null> {
    // Increment ref for parameters with full transfer ownership
...
return Promise.reject("Call failed");
  ...
  async get_static_pad_templates(): Promise<GLibList> {
    ...
    // Return value is a List, instantiate it from the ptr
if (data.return && typeof data.return === 'object' && 'ptr' in data.return) {
  const instance = await GLibList.create(data.return.ptr, 'none', GstStaticPadTemplate, undefined);
  return instance;
}
return Promise.reject("Call failed");
  ...
  async get_uri_protocols(): Promise<Array<string>> {
    // Increment ref for parameters with full transfer ownership
