...
export class GstDeviceMonitor extends GstObject {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstDeviceMonitor> {
    const instance = new GstDeviceMonitor(ptr, transferType);
...
const instance = await GstDeviceMonitor.create(data.return.ptr, 'full');
        return instance;
      }
...
return Promise.reject("Call failed");
  ...
  async get_providers(): Promise<Array<string>> {
    // Increment ref for parameters with full transfer ownership
...
return Promise.reject("Call failed");
  ...
  async set_show_all_devices(show_all: boolean, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
