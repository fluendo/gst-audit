...
export class GstAtomicQueue {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstAtomicQueue> {
    const instance = new GstAtomicQueue(ptr, transferType);
    if (transferType === 'full')
      gstatomicqueueRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GstAtomicQueue.create(data.return.ptr, 'full');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async ref(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
