...
export class GstPadProbeInfo {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstPadProbeInfo> {
    const instance = new GstPadProbeInfo(ptr, transferType);
    if (transferType === 'full')
      gstpadprobeinfoRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GstPadProbeInfo.create(data.return.ptr, 'none');
  return instance;
}
...
const instance = await GstEvent.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
...
const instance = await GstQuery.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
