...
export class GstParseContext {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstParseContext> {
    const instance = new GstParseContext(ptr, transferType);
    if (transferType === 'full')
      gstparsecontextRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GstParseContext.create(data.return.ptr, 'full');
  return instance;
}
...
return Promise.reject("Call failed");
  ...
  async get_missing_elements(): Promise<Array<string> | null> {
    // Increment ref for parameters with full transfer ownership
