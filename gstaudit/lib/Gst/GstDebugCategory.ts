...
export class GstDebugCategory {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstDebugCategory> {
    const instance = new GstDebugCategory(ptr, transferType);
    if (transferType === 'full')
      gstdebugcategoryRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GstDebugCategory.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async reset_threshold(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
  ...
  async set_threshold(level: GstDebugLevelValue, Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
