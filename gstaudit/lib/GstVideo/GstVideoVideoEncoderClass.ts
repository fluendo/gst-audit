...
import type { GstVideopre_push } from './GstVideopre_push';
import { convertGstVideopre_pushArgs } from './GstVideopre_push';
...
export class GstVideoVideoEncoderClass extends GstElementClass {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GstVideoVideoEncoderClass> {
    const instance = new GstVideoVideoEncoderClass(ptr, transferType);
    return instance;
  }
  ...
  async get_element_class(): Promise<Pointer | null> {
    ...
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/element_class`, apiConfig.baseUrl);
    try {
  ...
  async get_open(): Promise<GstVideoopen | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/open`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_close(): Promise<GstVideoclose | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/close`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_start(): Promise<GstVideostart | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/start`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_stop(): Promise<GstVideostop | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/stop`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_set_format(): Promise<GstVideoset_format | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/set_format`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_handle_frame(): Promise<GstVideohandle_frame | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/handle_frame`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_reset(): Promise<GstVideoreset | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/reset`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_finish(): Promise<GstVideofinish | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/finish`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_pre_push(): Promise<GstVideopre_push | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/pre_push`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_getcaps(): Promise<GstVideogetcaps | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/getcaps`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_sink_event(): Promise<GstVideosink_event | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/sink_event`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_src_event(): Promise<GstVideosrc_event | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/src_event`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_negotiate(): Promise<GstVideonegotiate | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/negotiate`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_decide_allocation(): Promise<GstVideodecide_allocation | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/decide_allocation`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_propose_allocation(): Promise<GstVideopropose_allocation | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/propose_allocation`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_flush(): Promise<GstVideoflush | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/flush`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_sink_query(): Promise<GstVideosink_query | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/sink_query`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_src_query(): Promise<GstVideosrc_query | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/src_query`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get_transform_meta(): Promise<GstVideotransform_meta | null> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/transform_meta`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
  ...
  async get__gst_reserved(): Promise<Array<Pointer>> {
    // Increment ref for parameters with full transfer ownership
    const url = new URL(`${apiConfig.normalizedBasePath}/GstVideo/VideoEncoderClass/ptr,${this.ptr}/fields/_gst_reserved`, apiConfig.baseUrl);
    try {
      // Auto-inject correlation ID if we're in a callback context
      const correlationId = getActiveCorrelationId();
      const headers: Record<string, string> = {};
