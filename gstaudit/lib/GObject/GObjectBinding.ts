...
export class GObjectBinding extends GObjectObject {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GObjectBinding> {
    const instance = new GObjectBinding(ptr, transferType);
...
const instance = await GObjectObject.create(data.return.ptr, 'none');
  ...
  async unbind(Prefer?: string): Promise<void> {
    // Increment ref for parameters with full transfer ownership
