...
export class GObjectValueArray {
  ...
  static async create(ptr: string, transferType: transferType): Promise<GObjectValueArray> {
    const instance = new GObjectValueArray(ptr, transferType);
    if (transferType === 'full')
      gobjectvaluearrayRegistry.register(instance, ptr);
    return instance;
  }
...
const instance = await GObjectValueArray.create(data.return.ptr, 'full');
  return instance;
}
return Promise.reject("Call failed");
  ...
  async append(value_?: GObjectValue): Promise<GObjectValueArray> {
    ...
    // Return value is a struct, instantiate it from the ptr
if (data.return && typeof data.return === 'object' && 'ptr' in data.return) {
const instance = await GObjectValueArray.create(data.return.ptr, 'none');
  return instance;
}
return Promise.reject("Call failed");
...
return Promise.reject("Call failed");
  ...
  async sort(session_id: string, callback_secret: string, compare_func: GLibCompareDataFunc): Promise<GObjectValueArray> {
    // Increment ref for parameters with full transfer ownership
