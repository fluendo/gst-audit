...
export function useCallbackRegistry(
  ...
  // Use a ref to store the sendMessage function to avoid circular dependency
  const sendMessageRef = useRef<((message: unknown) => void) | null>(null);

  // Handle callback messages - this callback is stable
  const handleMessage = useCallback(async (message: WebSocketMessage) => {
    ...
    // Send result back to server via WebSocket using the ref
    try {
